jQuery(function(){
    ...
    uploader.on("uploadBeforeSend",function (obj,data,headers) {
        ...
        data.works = $("#works").val();
        $(".ckbx-circle-4").find(":checkbox").each(function () {
            var i;
            var s = {};
            i = 'tag_'+$(this).val();
            if($(this).is(':checked')){
    ...
    function addFile( file ){
        ...
        var imgRight = (index+1) * (thumbnailWidth+10);
        var imgTop = 0;
        var wrapHeight = thumbnailHeight+20;
        var wrapWidth = $queue.width() +20 + 10;
        if( imgRight >= wrapWidth){
            imgTop = parseInt(index/parseInt(wrapWidth/(thumbnailWidth+10)))*wrapHeight;
            console.log(imgTop);
            wrapHeight = imgTop + wrapHeight;
            imgRight = (index % (parseInt(wrapWidth/(thumbnailWidth+10)) ) +1) * (thumbnailWidth+10);
        }
        $queue.height(wrapHeight);
        var $li = $('<li data-key="'+file.key+'"  data-src="'+file.src+'" data-sort="'+index+'" draggable="true" id="' + file.id + '" style="position:absolute;margin:0;cursor:move;width:'+thumbnailWidth+'px;height:'+thumbnailHeight+'px;left:'+(imgRight-thumbnailWidth)+'px;top:'+imgTop+ 'px;">' +
                '<p class="title">' + file.name + '</p>' +
                '<p class="imgWrap"></p>' + 
                '<p class="progress"><span></span></p>' + '</li>'
            ),
            $btns = $('<div class="file-panel">' +
                '<span class="cancel">delete</span>').appendTo( $li ),
